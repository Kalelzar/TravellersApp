...
Date Date::fromString(const char str[11]) { return {str}; }

int Date::daysBetween(const Date &start, const Date &end) {
  return start.daysBetween(end);
}

int Date::daysSince1900(const Date &date) { return date.daysSince1900(); }
...
static bool isDash(std::istream &in) { return in.get() == '-'; }
...
bool Date::operator<(Date const &other) const { return !(*this >= other); }
...
bool Date::operator==(Date const &other) const {
  return other.getYear() == getYear() && other.getMonth() == getMonth() &&
         other.getDay() == getDay();
}
