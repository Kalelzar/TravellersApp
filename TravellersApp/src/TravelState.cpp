...
void Travel::TravelState::userVisited(const char* dest,
                                      const char* name){
...
void Travel::TravelState::friendVisited(const char* dest) {
    ...
    fr.foreach([&dest,this](char* const(&f)){
                   userVisited(dest, f);
               });
}
...
void Travel::TravelState::destinationComments(const char *dest) {
      ...
      ->foreach([&dest, this](User *const(&u)){
                    userVisited(dest, u->getName());
                });
}

void Travel::TravelState::destinationRating(const char* dest){
    auto ratings = users.values()
        ->filter([&dest](User* const(&u)){
                     return u->hasVisited(dest);
                 })
    ...
    int sum = ratings->fold(0, [](int const& a, int const& b){
                               return a + b;
                               });
    std::cout<<"Average rating of "<<dest<<" is "
             <<(((double) sum)/ratingCount)
             <<std::endl;
}
