...
Travel::CommandList::CommandList() { LOG(INFO, "Create CommandList."); }
...
Travel::CommandList &
Travel::CommandList::operator=(Travel::CommandList const &other) {
  if (&other != this) {
    copy(other);
  }
  return *this;
...
void Travel::CommandList::printCommandsWithDescription() {
  std::cout << "Simple Command-line interface for Travel Management"
            << std::endl;
...
std::unique_ptr<Nullable<ScannerContext>>
Travel::CommandList::contextFor(TokenType tokenType) {
  LOG(VERBOSE, "Get context for token type");
  return commandMap.get(tokenType);
}

std::unique_ptr<Nullable<TokenType>>
Travel::CommandList::tokenFor(SimpleString const &name) {
