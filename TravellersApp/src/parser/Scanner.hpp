 ...
 *
 * You can provide a list of reserved keywords with {@link Travel::CommandList
 CommandList}.
 * Some keywords may only be provided in a certain context (File, Command-line
 or all)
 * as such the scanner needs to know it's own context which is provided at
 instantiation
...
public:
   ...
   *
   * The last token of the returned list is always of type {@link
   * TokenType::EOF_T EOF}
   *
   * @return a shared pointer to a list containing the resulting tokens
   */
