...
Travel::Scanner::Scanner(ScannerContext sc, std::istream *input,
                         CommandList *cl) {
    ...
    if (line[index] == '"') {
      ...
      while (line[index] != '"') {
        ...
        if (index >= linelen) {
          ...
          list->append({TokenType::ERROR, errorMsg, this->line});
          error = true;
          return list;
        }
      }
      ...
      while (isdigit(line[index])) {
        ...
        if (line[index] == '-' && tt == TokenType::NUMBER &&
            index + 1 < linelen && isdigit(line[index + 1])) {
          tt = TokenType::DATE;
          index++;
          maybeDate = true;
        } else if (line[index] == '-' && maybeDate && index + 1 < linelen &&
                   isdigit(line[index + 1])) {
          index++;
          maybeDate = false;
        } else if (line[index] == '-') {
      ...
      if (tt->isDefined()) {
        ...
        ScannerContext tctx =
            cl->contextFor(tt->get())->getOrElse(ScannerContext::UNDEFINED);
        if (tctx == ScannerContext::ALL || tctx == sc ||
            sc == ScannerContext::ALL) {
