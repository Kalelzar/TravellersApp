...
public:
  ...
  void append(A &&elem) override { insert(std::move(elem), length()); }

  void prepend(A const &elem) override { insert(elem, 0); }

  void prepend(A &&elem) override { insert(std::move(elem), 0); }
  ...
  /**
   * Slices the array in parts of the provided size.
   * Those slices are stored in a tuple and an array list of all
   * the resulting tuples is returned.
   *
   */
  ...
  slice() {
    ...
    auto list = std::make_unique<
        ArrayList<typename Tuple<A>::template ofSize<sliceSize>::type>>(slices);

    for (int i = 1; i < slices; i++) {
      typename Tuple<A>::template ofSize<sliceSize>::type tuple(elems +
                                                                sliceSize * i);
      list->append(tuple);
    }
  ...
  /**
   * Return the result of applying the provided function to the result of the
   * previous application or the provided starting value and the next element
   * in the list until all the elements of the list have been consumed.
   * @param start the starting value
   * @param biconsumer the function to apply
  ...
  /**
   * Return a list of pairs where each pair is composed of the elements of the
   * two lists with the same index. The lists must have the same amount of
   * elements
   */
  template <typename B>
...
template <typename A>
static std::ostream &operator<<(std::ostream &out, ArrayList<A> list) {
  list.foreach ([&out](A const &a) { out << a << " "; });
  return out;
}
