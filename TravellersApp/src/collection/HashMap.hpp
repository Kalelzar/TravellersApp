...
private:
   ...
   */
  double loadFactor() const { return length() / (double)capacity(); }
  ...
  void putAll(const SpecEntry *other, unsigned size) {
    for (unsigned i = 0; i < size; i++) {
      if (!other[i])
        continue; // Skip if the entry is not initialized
...
public:
  ...
  std::unique_ptr<Nullable<Value>> get(Key const &k) {
    ...
    if (!array[hsh]) {
      ...
      return std::make_unique<NotNull<Value>>(array[hsh]->value);
  }
  ...
  /**
   * Move the given key-value pair in the HashMap.
   * If the key is already in the HashMap, its value will be updated.
   * @param k the key
   * @param v the value
   */
  ...
  /**
   * Removes the given key from the hash map by setting it's corresponding
   * entry's as uninitialized. This will probably free any memory associated
   ...
   * @return the entries
   */
  const SpecEntry *underlying() const { return array; }

  /// Return a list of the key-value pairs stored in this HashMap
  std::unique_ptr<ArrayList<Tuple<Key, Value>>> toList() {
    auto list = std::make_unique<ArrayList<Tuple<Key, Value>>>(length());
    for (int i = 0; i < capacity(); i++) {
      if (array[i]) {
        list->append(
            std::move(Tuple<Key, Value>(array[i]->key, array[i]->value)));
      }
    }
    return list;
  }
